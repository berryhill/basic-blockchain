...
const dbFile = "db.db"
const genesisCoinbaseData = "arbitary_value"
...
func (cli *CLI) Run() {
	...
	createBlockchainCmd := flag.NewFlagSet(
		"createblockchain", flag.ExitOnError)
...
func NewUTXOTransaction(
	...
	if acc < amount {
		log.Printf("ERROR: Not enough funds")
		os.Exit(1)
	}
	...
	if acc > amount {
		outputs = append(
			outputs, TXOutput{acc - amount, from})
	}
...
func (tx Transaction) IsCoinbase() bool {
	...
	return len(tx.Vin) ==
		1 && len(tx.Vin[0].Txid) == 0 && tx.Vin[0].Vout == -1
}
